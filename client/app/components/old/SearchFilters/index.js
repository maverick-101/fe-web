...
class SearchFilters extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      units: [
        // {__html: 'Standard', value: 'standard', selected: 'selected'},
        {__html: 'Marla', value: 'marla', selected: 'selected'},
    ...
    .then(() => {
      ...
      var selectedAreas = this.props.query.area_id && typeof(this.props.query.area_id) != 'string' && this.props.query.area_id.length ? 
        ...
        //  if(checkSizeValid){this.props.dispatch(searchProperty())}
  ...
  getAreas(city) {
    ...
    if (!city || !city.value) {
      ...
      this.search({...queryParams, city_id: undefined, area_id: undefined, area_names: undefined, city: undefined, city_name: undefined})
  ...
  setAreas(areas) {
    ...
    var area_names = areas.map(area => area.label);
        this.props.dispatch(
          getAreas({city_id: this.props.query.city_id})
          )
        .then(() => {
          this.props.dispatch(removeAreas())
        })
  ...
  search(filter) {
    ...
    //comment this line
    this.props.dispatch(removeAreas())
    //uncomment this line if issues
    filter.city_id ? this.props.dispatch( getAreas({city_id: filter.city_id}) ).then(() => {this.props.dispatch(removeAreas())}) : null
  }
  startSearch() {
    this.props.dispatch(setSearchAreas())
  ...
  render() {
    ...
    this.props.query.type == "residential" ? (
      propertyFilters1 = ['central_heating', 'central_cooling', 'dirty_kitchen', 'lawn', 'swimming_pool', 'parking_space'],
    ...
    this.props.query.type == "residential" ? (
      mobilePropertyFilters = ['central_heating', 'central_cooling', 'dirty_kitchen', 'lawn', 'swimming_pool', 'wifi', 'balcony', 'laundry_room', 'servant_quarter']
    ) : ( this.props.query.type == "plot" ? (
        ...
        <div className="container-fluid">
          ...
          <div className={`visible-xs col-xs-12`}>
            ...
            <div className={`col-sm-12 no-padding`} id="filterRow">
              ...
              <div className={`${style.smallmargin}`} style={{width: '100%', marginBottom: '40px', display: 'inline-block', }}>
              <Counter label="Bathroom" sideStyle value={this.state.bath} onChange={event=> {
                    this.onChange({
                      target: {
                        name: 'bath',
